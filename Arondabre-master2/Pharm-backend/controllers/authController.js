...
exports.getLoggedInUser = async (req, res, next) => {
  try {
    ...
    // First try USER
    let user = await User.findOne({ account: accountId }).populate("account");
    if (user) {
      return res.status(200).json({ result: user });
    }

    // Then try SELLER
    let seller = await Seller.findOne({ account: accountId }).populate("account");
    if (seller) {
      return res.status(200).json({ result: seller });
    }

    // Then try DOCTOR
    let doctor = await Doctor.findOne({ account: accountId }).populate("account");
    if (doctor) {
